---
title: "vbhavya_FinalHomeworkCode_04"
...
z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, alternative = "two.sided", conf.level = 0.95){
  if (is.null(p2) == T | is.null(n2) == T){ ## had to change this because apparently I was using the missing() incorrectly
        p2 <- NULL  
        n2 <- NULL
    ...
    #Not gonna lie, I completely ripped this return bit from Sam. I will explain why below. 
...
### Response to comments: Thanks! I did make this change. I also saw that Sam did this in her assignment too. So I made the change.

### our one-sample z prop test equations and if(alternative == "greater") (or less) are the same but i specified before those two lines of code (64-68) the following code:if(alternative == "two.sided") {p <- 1 - pnorm(z, lower.tail = TRUE) + pnorm(z, lower.tail = FALSE)}
...
#if(alternative == "two.sided") {p <- 1 - pnorm(z, lower.tail = TRUE) + pnorm(z, lower.tail = FALSE)} is essentially the same as 2*pnorm (z, lower.tail = FALSE). I just simplified it. I don't think either of us are wrong
```

```{r testing the function makes me cry}
### from sofia: i would like to come back and test this to make sure it works!! i will leave it for now but hopefully by the end of tomorrow (monday) i will be able to check this by hand to see if it works. it looks good though!! 

### Response: Yeah, I ran this. :)

z.prop.test(0.21, 195, 0.58, 605, 0.5, alternative = "two.sided", conf.level = 0.95)

#So, when I ran this with my previous code, I just got a warning message that NaNs were being produced and it just didn't give me a result. Once I ripped off Sams code for returning values, I figured out that my function isn't calculating the CIs. Not sure what to do but at least I know that this is the issue?
```
...
g <- g+ geom_text(x = 350, y = 250, label = equation)
g
...
### Response to comments: Yeah, I had initially not done it, but then my code wouldn't run because at some point it produced an error message about NAs being present? I don't remember. But yeah, I had to try a lot of things before I found the NA problem. 
...
g <- g + geom_point()
g <- g + geom_smooth(method = "lm", formula = y ~ x)
g <- g+ geom_text(x = 5, y = 5.4, label = equation) ###Full transparency, I ripped off Sam again. I understood the code though, I promise. 
g 
```
...
From sofia: our code looks the same for calculating CI, but our values are slightly different (i think it's because i didn't use na.omit initially). I think interpreting the point estimate just means to calculate it? (i also wasn't sure lmao)
...
#Plotting: 
colors <- c("Fit" = "black", "Confidence" = "red", "Prediction" = "green")


g <- ggplot(intervals, aes(x = brain_size, y= longevity))+ 
  geom_point()+
  geom_line(aes(x = brain_size, y = fit, color = "Fit"))+
  geom_line(aes(brain_size, lwr, color = "Prediction"))+
  geom_line(aes(brain_size, upr, color = "Prediction"))+
  geom_line(aes(brain_size, lower_longevity, color = "Confidence"))+
  geom_line(aes(brain_size, upper_longevity, color = "Confidence"))+
...
### Response: Yeah, I'm not sure what's on with the colors. It's weird. I've changed the code to add a legend now. So, that's no longer an issue. 
```
...
g <- ggplot(intervals_log, aes(x = log_brain_size, y= log_longevity))+ 
  geom_point()+
  geom_line(aes(x = log_brain_size, y = fit, color = "Fit"))+
  geom_line(aes(log_brain_size, lwr, color = "Prediction"))+
  geom_line(aes(log_brain_size, upr, color = "Prediction"))+
  geom_line(aes(log_brain_size, log_lower_longevity, color = "Confidence"))+
  geom_line(aes(log_brain_size, log_upper_longevity, color = "Confidence"))
...
I've addressed some of the comments in the chunks themselves. But overall, I found the comments really helpful. This homework assignment was quite dense and I think it really helped having someone look over the homework assignment and keep track of things with me. I for sure would be so lost without the peer review process. Thanks!